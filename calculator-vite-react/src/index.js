...
buttonsOperationsAndNumber.forEach((element) => {
  element.addEventListener('click', () => {
    ...
    if (element.id != 'dot' && element.id != 'operation') {
      inputOperation.value = inputOperation.value
        ? inputOperation.value + element.textContent
        : element.textContent;
    }
  });
});
...
const resolveOperation = () => {
  switch (operation) {
    case 'x':
      inputOperation.value =
        (Number.isInteger(numberOne) ? parseInt(numberOne) : parseFloat(numberOne)) *
  ...
  inputOperation.value = inputOperation.value.includes('.')
    ? String(inputOperation.value).replace('.', ',')
    : eval(inputOperation.value);
